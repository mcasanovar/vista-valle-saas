...
El sistema SHALL detectar cuando un evento entrante se superpone con una reserva u retención vigente ya existente para la misma habitación y fechas, SHALL descartar la creación de la reserva conflictiva sin alterar la reserva u retención existente, y SHALL generar una alerta operativa identificando el conflicto.
...
El sistema SHALL generar la misma alerta operativa de conflicto cuando una retención de pago en línea vence y su reserva no puede confirmarse porque, en el intervalo, una sincronización externa ocupó la habitación para las mismas fechas.
