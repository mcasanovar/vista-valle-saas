...
- **THEN** el sistema entrega el estado habilitado/deshabilitado vigente de cada método
...
- **THEN** el sistema garantiza que exista una entrada con ambos métodos habilitados por defecto en el primer acceso, sin requerir una creación manual previa ni devolver una lectura vacía

### Requirement: Edición de métodos de pago desde el dashboard admin
El sistema SHALL permitir a un administrador autenticado habilitar o deshabilitar independientemente "pagar al llegar" y "pagar online" desde el dashboard admin, SHALL permitir cualquier combinación incluyendo ambos deshabilitados sin exigir que al menos uno quede activo, y SHALL reflejar la actualización inmediatamente en las siguientes lecturas del flujo público.

#### Scenario: Actualización exitosa de un método
- **WHEN** un administrador autenticado deshabilita "pagar online" y guarda
- **THEN** el sistema actualiza la entrada de configuración y las siguientes consultas del flujo público de reservas reflejan que "pagar online" está deshabilitado

#### Scenario: Ambos métodos deshabilitados
- **WHEN** un administrador autenticado deshabilita "pagar al llegar" y "pagar online" y guarda
